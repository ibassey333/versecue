...
export default function SessionEditorPage({ 
  ...
  const handleDownloadTranscript = () => {
    console.log('[Download] Transcript clicked', session?.title);
    if (!session || !session.transcript) {
      console.log('[Download] No transcript');
      return;
    }
    ...
    try {
      ...
      const content = [
        ...
        '',
        '─'.repeat(50),
        `Generated by VerseCue • ${new Date().toLocaleDateString()}`,
      ].join('\n');
      
      // Create and download blob
      const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
      saveAs(blob, `${session.title.replace(/[^a-z0-9]/gi, '_')}_transcript.txt`);
    } catch (error) {
  ...
  // Premium PDF Download with optional transcript
  const handleDownloadPDF = () => {
    console.log('[Download] PDF clicked', session?.title, 'Include transcript:', includeTranscript);
    if (!session) {
    ...
    try {
      ...
      // ===== TRANSCRIPT APPENDIX (if enabled) =====
      if (includeTranscript && session.transcript) {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(26, 54, 93);
        doc.text('Full Transcript', pageWidth / 2, yPos, { align: 'center' });
        yPos += 6;
        ...
        // Determine layout: single or double column based on length
        const transcriptLength = session.transcript.length;
        const useDoubleColumn = transcriptLength > 3000;
        ...
        if (useDoubleColumn) {
          // Double column layout for long transcripts
          ...
          paragraphs.forEach((para) => {
            const currentX = useLeft ? margin : margin + colWidth + colGap;
            const currentY = useLeft ? leftY : rightY;
            
            // Check if we need a new page
            if (currentY > pageHeight - 25) {
              if (useLeft && rightY < pageHeight - 25) {
            ...
            const paraLines = doc.splitTextToSize(para.trim(), colWidth);
            doc.text(paraLines, currentX, useLeft ? leftY : rightY);
            
            const lineHeight = paraLines.length * 3.8 + 4;
            if (useLeft) {
              leftY += lineHeight;
              if (leftY > pageHeight - 30) useLeft = false;
            } else {
              rightY += lineHeight;
          ...
          // Single column for shorter transcripts
          const paragraphs = session.transcript.split(/\n\n|\n/).filter(p => p.trim());
  ...
  // Premium Word Download with optional transcript
  const handleDownloadWord = async () => {
    console.log('[Download] Word clicked', session?.title, 'Include transcript:', includeTranscript);
    if (!session) {
      console.log('[Download] No session');
      return;
    }
    ...
    try {
      ...
      // Build transcript section if needed
      const transcriptSection = (includeTranscript && session.transcript) ? [
        ...
        new Paragraph({
          children: [
            new TextRun({ text: "Full Transcript", bold: true, size: 36, color: "1a365d" }),
          ],
        ...
        ...session.transcript.split(/\n\n|\n/).filter(p => p.trim()).map((para) =>
          new Paragraph({
            children: [
              new TextRun({ text: para.trim(), size: 22, color: "555555" }),
            ],
              ...
              <div className="relative">
                ...
                {showDownloadMenu && (
                  <>
                    ...
                    />
                    {/* Dropdown menu */}
                    <div className="absolute right-0 top-full mt-2 w-56 bg-verse-surface border border-verse-border rounded-xl shadow-xl overflow-hidden z-[70]">
                      {/* Transcript checkbox */}
                      {session.transcript && (
                        <label 
                          className="flex items-center gap-3 px-4 py-3 border-b border-verse-border cursor-pointer hover:bg-verse-bg/50 transition-colors"
                        ...
                        >
                          <input
                            ...
                            className="w-4 h-4 rounded border-verse-border text-gold-500 focus:ring-gold-500 focus:ring-offset-0 bg-verse-bg"
                          />
                          <span className="text-sm text-verse-text">Include transcript</span>
                        </label>
                      )}
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div className="text-xs text-verse-muted">
                            {includeTranscript ? 'Summary + transcript' : 'Summary only'}
                          </div>
                        </div>
                      </button>
                      ...
                      {session.transcript && (
                        <button
                          ...
                          className="w-full flex items-center gap-3 px-4 py-3 text-left text-verse-text hover:bg-verse-border transition-colors cursor-pointer border-t border-verse-border"
                        >
                          <FileText className="w-4 h-4 text-verse-muted" />
                          <div>
                            <div className="font-medium">Transcript</div>
                            <div className="text-xs text-verse-muted">Plain text (.txt)</div>
                          </div>
                        </button>
                      )}
                    </div>
