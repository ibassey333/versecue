  ...
  return (
    <div className="mt-4 pt-4 border-t border-verse-border">
      ...
      <p className="mt-2 text-[10px] text-verse-muted">
        KJV works offline. WEB/ASV need API.Bible. Deepgram improves speech accuracy. Groq adds AI detection.
      </p>
    </div>
  );
}
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-verse-border">
        <div className="flex items-center gap-3">
          ...
          {pendingQueue.length > 0 && (
            <span className="flex items-center justify-center min-w-[24px] h-6 px-2 text-xs font-bold text-verse-bg bg-yellow-500 rounded-full">
              {pendingQueue.length}
            </span>
          )}
        </div>
          ...
          <div className="space-y-3">
            {pendingQueue.map((item) => (
              <div key={item.id} className="rounded-xl border border-yellow-500/30 bg-verse-elevated p-4">
                ...
                {item.verseText && <p className="text-xs text-verse-muted mb-3 line-clamp-2">{item.verseText}</p>}
                <div className="flex items-center gap-2">
                  <button onClick={() => approveDetection(item.id)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-green-500 text-white text-sm font-medium hover:bg-green-600 transition-colors">
                    <Check className="w-4 h-4" /> Approve
                  </button>
                  <button onClick={() => dismissDetection(item.id)} className="px-3 py-2 rounded-lg bg-verse-border text-verse-subtle text-sm hover:bg-verse-elevated transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              </div>
        ...
        {approvedQueue.length === 0 ? (
          ...
          <div className="space-y-3">
            {approvedQueue.map((item) => (
              <div key={item.id} className={cn('rounded-xl border p-4 transition-all', item.displayedAt ? 'border-verse-border/50 bg-verse-bg/50' : 'border-green-500/30 bg-verse-elevated')}>
                <div className="flex items-center justify-between mb-2">
                  <h4 className={cn('font-display text-lg font-semibold', item.displayedAt ? 'text-verse-muted' : 'text-green-500')}>{item.reference.reference}</h4>
                  <button onClick={() => removeFromApproved(item.id)} className="p-1 text-verse-muted hover:text-red-500 transition-colors"><X className="w-4 h-4" /></button>
                </div>
                {item.verseText && <p className="text-xs text-verse-muted mb-3 line-clamp-2">{item.verseText}</p>}
                <div className="flex items-center justify-between">
                  <button onClick={() => item.displayedAt ? redisplayScripture(item.id) : displayScripture(item.id)} className={cn('flex items-center gap-2 px-4 py-2 rounded-lg font-semibold text-sm transition-all', item.displayedAt ? 'bg-verse-border text-verse-text hover:bg-verse-elevated' : 'bg-gold-500 text-verse-bg hover:bg-gold-400')}>
                    {item.displayedAt ? 'Re-display' : 'Display'} <kbd className="text-[10px] opacity-60">↵</kbd>
                  </button>
                  {item.displayedAt && <span className="text-[10px] text-green-500">✓ Shown {formatTime(item.displayedAt)}</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        {currentDisplay ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center">
            ...
            <div className="flex items-center gap-2 mt-4">
              <button onClick={goToPrevVerse} className="p-2 rounded-lg bg-verse-surface text-verse-muted hover:text-verse-text transition-colors" title="Previous verse (←)">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <button onClick={goToNextVerse} className="p-2 rounded-lg bg-verse-surface text-verse-muted hover:text-verse-text transition-colors" title="Next verse (→)">
                <ChevronRight className="w-5 h-5" />
              </button>
            </div>
            
            <button onClick={clearDisplay} className="absolute top-3 right-3 p-2 rounded-lg bg-verse-surface/80 text-verse-muted hover:text-verse-text text-sm transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        ) : (
    ...
    <div className="rounded-xl border border-verse-border bg-verse-surface p-4">
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input 
          ...
          className="flex-1 px-4 py-3 rounded-xl bg-verse-bg border border-verse-border text-verse-text placeholder:text-verse-muted text-sm focus:outline-none focus:ring-2 focus:ring-gold-500/50" 
        />
        <button
          type="submit"
          disabled={loading || !query.trim()}
          className={cn(
            'px-4 py-3 rounded-xl bg-gold-500 text-verse-bg font-medium text-sm transition-all hover:bg-gold-400',
      ...
      <p className="mt-2 text-[11px] text-verse-muted">
        Press <kbd className="px-1.5 py-0.5 rounded bg-verse-border font-mono text-[10px]">/</kbd> to focus • Try "Psalm chapter 23 verse 1"
      </p>
    </div>
  );
}
