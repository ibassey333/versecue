...
function SongSearch({ onSelect }: { onSelect: (song: Song) => void }) {
  ...
  const [searchSource, setSearchSource] = useState<'all' | 'library' | 'lrclib'>('library');
...
function LyricsPreview({ orgSlug }: { orgSlug?: string }) {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        console.log('[LyricsPreview] Error fetching settings:', err);
      }
    };
    fetchSettings();
  }, [org?.id]);
  ...
  const settings = displaySettings || {
    ...
    padding: 48,
    preview_title_color: '#fbbf24',
    preview_title_size: 14,
    preview_part_color: '#fbbf24',
    preview_part_size: 12,
    preview_shadow_intensity: 2,
  };
  ...
  const buildTextShadow = (): string | undefined => {
    ...
    if (settings.text_outline && settings.text_outline_width > 0) {
      const outline = generateTextOutline(
        settings.text_outline_width * scaleFactor,
        settings.text_outline_color || '#000000'
      );
      if (outline) shadows.push(outline);
    }
    if (settings.text_shadow) {
      shadows.push('1px 1px 2px rgba(0,0,0,0.5)');
      shadows.push('0 0 10px rgba(0,0,0,0.3)');
    }
  ...
  return (
    <div className="rounded-xl border border-verse-border bg-verse-surface overflow-hidden">
      ...
      <div 
        className={`relative aspect-video flex flex-col ${verticalAlignClass} overflow-hidden`}
        style={{
          ...
          >
            <p 
              ...
              style={{
                ...
                fontSize: settings.preview_title_size,
                textShadow: settings.preview_shadow_intensity === 0 ? 'none' :
                  settings.preview_shadow_intensity === 1 ? '0 1px 2px rgba(0,0,0,0.5)' :
                  settings.preview_shadow_intensity === 2 ? '0 2px 4px rgba(0,0,0,0.7)' :
                  '0 2px 6px rgba(0,0,0,0.9), 0 0 10px rgba(0,0,0,0.5)',
              }}
            >
            ...
            <pre 
              ...
              style={{ 
                fontFamily,
                fontSize: Math.max(10, settings.verse_font_size * scaleFactor),
            ...
            {currentSection.isSplitPart && (
              ...
              style={{
                color: settings.preview_part_color,
                fontSize: settings.preview_part_size,
                textShadow: settings.preview_shadow_intensity === 0 ? 'none' :
                  settings.preview_shadow_intensity === 1 ? '0 1px 2px rgba(0,0,0,0.5)' :
                  settings.preview_shadow_intensity === 2 ? '0 2px 4px rgba(0,0,0,0.7)' :
                  '0 2px 6px rgba(0,0,0,0.9), 0 0 10px rgba(0,0,0,0.5)',
      ...
      {song && (
        <div className="px-5 py-3 border-t border-verse-border bg-verse-bg/50 flex items-center justify-between">
          ...
          <span className="text-xs font-medium text-verse-text">
            {isStaged ? (
              <span className="text-verse-muted">Click section to display</span>
            ) : (
              <>
...
export function WorshipPanel({ orgSlug }: { orgSlug?: string }) {
  ...
  const [worshipSettings, setWorshipSettings] = useState({
  ...
  // Fetch display settings for worship lyrics config (direct from Supabase)
  useEffect(() => {
    const fetchWorshipSettings = async () => {
      if (!org?.id) return;
      ...
      try {
        ...
        const { data, error } = await supabase
          .from('display_settings')
          .select('worship_auto_split, worship_max_lines, worship_max_chars')
          .eq('organization_id', org.id)
          .single();
        
        if (data && !error) {
          const newSettings = {
            autoSplitEnabled: data.worship_auto_split ?? true,
            maxLines: data.worship_max_lines ?? 4,
            maxChars: data.worship_max_chars ?? 150,
          };
          console.log('[Worship] Loaded settings:', newSettings);
          setWorshipSettings(newSettings);
        } else {
          console.log('[Worship] No settings found, using defaults');
        }
      } catch (err) {
        console.log('[Worship] Error fetching settings:', err);
      }
      setSettingsLoaded(true);
    };
    
    fetchWorshipSettings();
  }, [org?.id]);

  // Re-process current song when settings change (after initial load)
  useEffect(() => {
    if (!settingsLoaded) return;
    if (!worship.currentSong) return;
    
    console.log('[Worship] Re-processing song with settings:', worshipSettings);
    setCurrentSong(worship.currentSong, {
      maxLinesPerPart: worshipSettings.maxLines,
      maxCharsPerPart: worshipSettings.maxChars,
      autoSplitEnabled: worshipSettings.autoSplitEnabled,
    });
  }, [settingsLoaded, worshipSettings.autoSplitEnabled, worshipSettings.maxLines, worshipSettings.maxChars]);
